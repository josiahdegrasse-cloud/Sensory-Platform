import { SLATE_200, SLATE_700, SLATE_950, setText, type PdfContext } from './theme';
...
export function renderBarChart(ctx: PdfContext, entries: BarChartEntry[], x: number, y: number, width: number) {
  ...
  entries.forEach(entry => {
    ...
    setText(doc, SLATE_700, 9.5, 'bold');
    ...
    doc.roundedRect(x + labelWidth, currentY, trackWidth, barHeight, 3, 3, 'F');
    const filled = Math.max(trackWidth * ratio, ratio > 0 ? 8 : 0);
    doc.setFillColor(...accent);
    doc.roundedRect(x + labelWidth, currentY, filled, barHeight, 3, 3, 'F');
