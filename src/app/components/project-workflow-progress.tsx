...
const STATE_STYLES: Record<WorkflowStageState, { chip: string; dot: string; connector: string; label: string }> = {
  ...
  available:     { chip: 'bg-slate-50 text-slate-700 hover:bg-slate-200', dot: 'bg-white text-slate-500 border-2 border-slate-200', connector: 'bg-slate-200', label: 'text-slate-500 group-hover:text-slate-700' },
  blocked:       { chip: 'bg-slate-50 text-slate-500 cursor-not-allowed', dot: 'bg-slate-50 text-slate-500 border border-slate-200', connector: 'bg-slate-50', label: 'text-slate-500' },
  'not-started': { chip: 'bg-slate-50 text-slate-500 hover:bg-slate-50', dot: 'bg-white text-slate-300 border-2 border-dashed border-slate-200', connector: 'bg-slate-50', label: 'text-slate-500 group-hover:text-slate-700' },
};
    ...
    <div className={cn('flex items-start', className)}>
      {stages.map((stage, i) => {
        ...
        const detailSpan = showDetails && (
          <span className="text-[11px] leading-snug text-slate-500 text-center max-w-[120px]">
            {stage.detail}
          </span>
        );
        return (
