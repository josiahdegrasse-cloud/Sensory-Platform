  ...
  return (
    <button
      ...
      className={cn(
        ...
        active ? 'bg-slate-100 ring-1 ring-inset ring-slate-300' : 'hover:bg-slate-50',
      )}
    >
      <span className="flex min-w-0 items-start justify-between gap-2">
        <span className="min-w-0">
          <span className={cn('block truncate text-sm font-semibold', active ? 'text-slate-950' : 'text-slate-900')}>
            {title}
          </span>
        ...
        <span className="flex shrink-0 items-start gap-1.5">
          ...
          {metric && (
            <span className="text-right">
              <span className="block text-sm font-bold tabular-nums text-slate-950">{metric}</span>
              {metricLabel && <span className="block text-[10px] font-semibold uppercase text-slate-400">{metricLabel}</span>}
            </span>
          )}
          {trailing}
        </span>
