...
export function PanelistInviteForm({ compact = false }: { compact?: boolean }) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setMessage(`Invitation sent to ${normalizedEmail}. They will appear in the box list after completing their account.`);
      setEmail('');
    } catch (err) {
  ...
  return (
    <div className={compact ? 'space-y-3' : 'rounded-lg border border-slate-200 bg-white p-4'}>
      {!compact && <div className="mb-3 flex items-start gap-3"><span className="flex size-9 shrink-0 items-center justify-center rounded-md bg-slate-100"><Mail className="size-4 text-slate-700" aria-hidden /></span><div><h3 className="text-sm font-bold text-slate-950">Invite a panelist</h3><p className="mt-0.5 text-xs leading-5 text-slate-600">Start with their email. They create the account and enter their own delivery details.</p></div></div>}
