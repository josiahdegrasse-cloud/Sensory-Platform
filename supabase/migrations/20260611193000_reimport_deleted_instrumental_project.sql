-- Re-importing a previously deleted or archived CSV must create a fresh active
-- project. Idempotency only suppresses retries while the original batch is
-- still active.

DROP INDEX IF EXISTS public.uq_import_batches_org_idempotency;
CREATE UNIQUE INDEX uq_import_batches_org_idempotency
