-- Store delivery addresses with named panelist boxes so packing, printing,
-- replacements, and fielding all use one durable source of truth. Postal
-- addresses remain admin-only and are not exposed by public invite RPCs.

ALTER TABLE public.panelist_kits
  ADD COLUMN IF NOT EXISTS recipient_address text;

CREATE OR REPLACE FUNCTION public.copy_panelist_kit_replacement_address()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
AS $$
BEGIN
  IF NEW.replacement_for_kit_id IS NOT NULL AND NEW.recipient_address IS NULL THEN
    SELECT source_kit.recipient_address
    INTO NEW.recipient_address
    FROM public.panelist_kits AS source_kit
    WHERE source_kit.id = NEW.replacement_for_kit_id
      AND source_kit.org_id = NEW.org_id;
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS copy_panelist_kit_replacement_address ON public.panelist_kits;
CREATE TRIGGER copy_panelist_kit_replacement_address
BEFORE INSERT ON public.panelist_kits
FOR EACH ROW
EXECUTE FUNCTION public.copy_panelist_kit_replacement_address();
...
BEGIN
  ...
  FOR v_i IN 1..kit_count LOOP
    ...
    INSERT INTO public.panelist_kits (
      ...
      recipient_name, recipient_email, recipient_address, created_by
    )
    VALUES (
      v_org,
      target_product_id,
